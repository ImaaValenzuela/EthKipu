document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (typeof ethers === 'undefined') {
        console.error('🔴 ethers no se cargó — revisa el script tag del CDN o usa otro CDN (jsdelivr / unpkg).');
        return;
    }
    if (typeof window === 'undefined' || typeof window.ethereum === 'undefined') {
        console.error('🔴 window.ethereum no disponible. ¿MetaMask está instalado y activado para este sitio?');
        return;
    }
    console.log('✅ ethers y window.ethereum detectados');
    console.log('ethers version (approx):', ethers?.utils ? 'OK' : 'unknown');

    // Prueba rápida de provider y cuenta
    const provider = new ethers.providers.Web3Provider(window.ethereum);
    try {
        const accounts = await window.ethereum.request({ method: 'eth_accounts' });
        console.log('Cuentas conectadas actualmente:', accounts);
    } catch (err) {
        console.warn('No se pudo obtener eth_accounts:', err);
    }
});
// Configuración
